...
func sendToBackQueue(chain *message.MessageChain, streamID string, queueMgr *WecomAIQueueMgr, streaming bool, suppressUnsupportedLog bool) string {
	...
	if chain == nil {
		if !queueMgr.trySendBackQueueItem(backQueue, &QueueItem{Type: "end", Data: "", Streaming: false}) {
	...
	for _, comp := range chain.Chain {
		...
		case *message.At:
			data = "@" + c.Name + " "
			if !queueMgr.trySendBackQueueItem(backQueue, &QueueItem{Type: "plain", Data: data, Streaming: streaming, SessionID: streamID}) {
		...
		case *message.Image:
			...
			if !queueMgr.trySendBackQueueItem(backQueue, &QueueItem{Type: "image", ImageData: imageBase64, Streaming: streaming, SessionID: streamID}) {
...
func markStreamComplete(streamID string, queueMgr *WecomAIQueueMgr) {
	backQueue := queueMgr.GetOrCreateBackQueue(streamID)
	if !queueMgr.trySendBackQueueItem(backQueue, &QueueItem{Type: "complete", Data: "", Streaming: false, SessionID: streamID}) {
