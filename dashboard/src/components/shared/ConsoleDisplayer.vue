  ...
  methods: {
    ...
    processNewLogs(newLogs) {
      ...
      if (hasUpdate) {
        ...
        let totalBytes = 0;
        for (const log of this.localLogCache) {
          totalBytes += String(log.data || "").length;
        }
        let skip = 0;
        const maxBytes = 1 << 20;
        while (totalBytes > maxBytes && skip < this.localLogCache.length - 1) {
          totalBytes -= String(this.localLogCache[skip].data || "").length;
          skip++;
        }
        if (skip > 0) {
