...
<template>
    ...
    <v-col v-if="activeTab === 'market'" cols="12" md="12">
      <div class="d-flex align-center justify-center mt-4 mb-4 gap-4">
        ...
        <v-btn
          ...
          href="https://github.com/WaterGodFurina/AstrBot-go-market"
          target="_blank"
          color="primary"
          class="text-none"
        >
  ...
  <v-dialog v-model="riskDialog.show" width="640" persistent>
    <v-card>
      <v-card-title class="text-h3 pa-4 pb-0 pl-6 d-flex align-center">
        ...
        {{ tm("dialogs.risk_warning.title") }}
      </v-card-title>
      <v-card-text>
        <div class="mb-2">
          {{ riskDialog.message || tm("dialogs.risk_warning.message") }}
        </div>
        <div
          v-if="riskDialog.risks.length"
          class="risk-finding-list"
          style="max-height: 300px; overflow-y: auto"
        >
          <div
            v-for="(finding, index) in riskDialog.risks"
            :key="index"
            class="risk-finding mb-2"
            style="
        ...
        <div class="text-medium-emphasis mt-2">
          {{ tm("dialogs.risk_warning.hint") }}
        </div>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="grey" variant="text" @click="cancelRiskInstall">
          {{ tm("dialogs.risk_warning.cancel") }}
        </v-btn>
        <v-btn color="error" variant="tonal" @click="confirmRiskInstall">
          {{ tm("dialogs.risk_warning.confirm") }}
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <!-- C 编译器选择对话框 -->
  <v-dialog v-model="ccDialog.show" width="560" persistent>
    <v-card>
      <v-card-title class="text-h3 pa-4 pb-0 pl-6 d-flex align-center">
        <v-icon color="warning" class="mr-2">mdi-language-cpp</v-icon>
        {{ tm("dialogs.c_compiler.title") }}
      </v-card-title>
      <v-card-text>
        <div class="mb-2">{{ ccDialog.message }}</div>
        <div v-if="ccDialog.hasGCC" class="mb-2">
          <div class="d-flex align-center">
            <v-chip size="small" color="success" variant="tonal" class="mr-2">
              GCC
            </v-chip>
            <code>{{ ccDialog.gccPath }}</code>
          </div>
          <div v-if="ccDialog.gccVersion" class="mt-1 text-caption">
            {{ ccDialog.gccVersion }}
          </div>
        </div>
        <div class="text-medium-emphasis">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="grey" variant="text" @click="cancelCCInstall">
          {{ tm("dialogs.c_compiler.cancel") }}
        </v-btn>
        <template v-if="ccDialog.hasGCC">
  ...
  <v-dialog v-model="dialog" width="500">
    ...
    >
      ...
      <div
        v-if="installProgress.show"
        class="install-progress pa-4 pb-0"
        style="padding-top: 0"
      >
        <v-progress-linear
          :model-value="installProgress.percent"
          color="primary"
          height="8"
          rounded
          striped
        ...
        <div class="d-flex align-center text-caption mt-2">
          ...
          {{ installProgress.text || tm("dialogs.install.preparing") }}
          <span v-if="installProgress.percent > 0">
