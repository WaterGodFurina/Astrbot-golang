...
export const useExtensionPage = (initialTab = "installed") => {
  ...
  // 插件源码风险警告对话框（静态扫描命中危险 import 时弹出）
  ...
  // 无视风险继续安装标记
  const ignoreRiskFlag = ref(false);

  // C 编译器选择对话框（插件声明 cgo 需要 C 工具链）
  ...
  let installId = String(Date.now()) + Math.random().toString(36).slice(2, 8);
  let pollTimer = null;
  const startInstallProgressPolling = () => {
    ...
    installProgress.value = {
      show: true,
      percent: 0,
      text: tm("dialogs.install.preparing") || "准备安装…",
    };    pollTimer = setInterval(async () => {
      try {
        const res = await pluginApi.installProgress(installId);
        const data = res?.data?.data;
        if (data && (data.status === "downloading" || data.status === "installing")) {
          installProgress.value.percent = data.percent || 0;
          if (data.text) installProgress.value.text = data.text;
        }
      } catch {
        // 轮询失败静默，下次重试
      }
  ...
  const stopInstallProgressPolling = () => {
    ...
    installProgress.value = { show: false, percent: 0, text: "" };
  };
  ...
  const confirmRiskInstall = async () => {
    riskDialog.show = false;
    ignoreRiskFlag.value = true;
    await newExtension();
  };

  const cancelRiskInstall = () => {
    riskDialog.show = false;
    riskDialog.risks = [];
    ignoreRiskFlag.value = false;
  };

  const chooseCCGCC = async () => {
    ccDialog.show = false;
    ccChoice.value = "gcc";
    await newExtension();
  };

  const chooseCCClang = async () => {
    ccDialog.show = false;
    ccChoice.value = "clang";
    await newExtension();
  };
  ...
  const cancelCCInstall = async () => {
    ccDialog.show = false;
    ccChoice.value = "cancel";
    await newExtension();
  };
  ...
  const newExtension = async (ignoreVersionCheck = false) => {
    const shouldIgnoreVersionCheck = ignoreVersionCheck === true;
    const shouldIgnoreRisk = ignoreRiskFlag.value;
    ignoreRiskFlag.value = false;
