...
export const providerApi = {
  ...
  async listByProviderType(
    ...
    const providers = responses
      .flatMap((response) => response.data.data.providers || [])
      .filter((provider) => {
...
export const pluginApi = {
  ...
  async installProgress(installId: string) {
    const response = await fetchWithAuth(
      `/api/v1/plugins/install/progress?install_id=${encodeURIComponent(installId)}`,
      {
        method: 'GET',
      },
    );
    ...
    if (!response.ok) {
      throw new Error(
        data?.message || `Failed to fetch install progress (${response.status})`,
      );
    }
    return { data } as AxiosResponse<ApiEnvelope<OpenConfig>>;
  },
